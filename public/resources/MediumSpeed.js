...
function parseMediumSpeed (msg, canIds, window) {
  ...
  if (IDs.includes(msg.id)) {
    if (msg.id === 888) {
      mediumSpeed.time.hour = (arr[5] - 128) < 10 ? '0' + (arr[5] - 128) : (arr[5] - 128)
    ...
    // 9A - Passenger Temp && Recric Vent
      ...
      mediumSpeed.temperature.passenger = (arr[7] - 128) / 2 < 16 ? 'LOW' : (arr[7] - 128) / 2 > 28 ? 'HIGH' : (arr[7] - 128) / 2
      mediumSpeed.temperature.driver = arr[6] / 2 < 16 ? 'LOW' : arr[6] / 2 > 28 ? 'HIGH' : arr[6] / 2
    }
  }
}
