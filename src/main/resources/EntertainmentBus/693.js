...
export function eb693(msg) {
	...
	if (arrData[0] == 16 || arrData[0] == 5) {
		// 16 & 5 mean start of new message
		messageBuilder = [];
	}
	...
	if (arrData[0] == 5) {
		...
		for (var i = 1; i < arrData.length; i++) {
			// Start at 1 to skip the 16 or 5
			if (arrData[i] == 0) {
				// 0 means end of message so parse it
...
function parseMessage(input) {
	...
	//5join("").toString(16)
	if (input[0] == 5) {
		// Do nothing... because I have no idea what it does or what it is for...It seems to mean reset packet... It works...
		if (input[0] == 75) {
			//K - Phone Name
