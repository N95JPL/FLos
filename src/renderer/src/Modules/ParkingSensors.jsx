...
function adjustSensorGraphic(sensorPosition, sensorGroup) {
	...
	trueId(`${sensorPosition}-stop1`).setAttribute(
		"style",
		`stop-color:${colourFinder(x)};stop-opacity:1;`
	);
	document
...
function ParkingSensors() {
	const sensorBLL = mediumSpeed((state) => state.rearLeft);
	const sensorBCL = mediumSpeed((state) => state.rearLeftMiddle);
	const sensorBCR = mediumSpeed((state) => state.rearRightMiddle);
	const sensorBRR = mediumSpeed((state) => state.rearRight);
	...
	return (
		<div className='items-center justify-center w-full h-full translate-x-[-45%]'>
			{/* only need to copy in this SVG and div and the logic from main.js with the call to the script*/}
			<div
				className='items-center top-6 justify-center w-[200%] h-[200%] absolute'
				id='SVG-div'>
				...
				<svg
					...
					<linearGradient id='backCenterLeft'>
						...
						<stop
							id='BCL-stop2'
							style={setSensorGraphicColour(0, sensorBCL)}
							offset='100%'
						/>
					</linearGradient>
					<radialGradient
					...
					<linearGradient id='backLeftLeft'>
						...
						<stop
							id='BLL-stop2'
							style={setSensorGraphicColour(0, sensorBLL)}
							offset='100%'
						/>
					</linearGradient>
					<radialGradient
					...
					<linearGradient id='backRightRight'>
						...
						<stop
							id='BRR-stop2'
							style={setSensorGraphicColour(0, sensorBRR)}
							offset='100%'
						/>
					</linearGradient>
					<radialGradient
					...
					<linearGradient id='backCenterRight'>
						...
						<stop
							id='BCR-stop2'
							style={setSensorGraphicColour(0, sensorBCR)}
							offset='100%'
						/>
					</linearGradient>
					<radialGradient
